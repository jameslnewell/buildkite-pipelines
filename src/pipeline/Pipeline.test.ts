...
describe("Pipeline", () => {
  ...
  test.only("secrets-docker-login-docker-cache", () => {
    const plugins = [
      SecretsManagerPlugin.builder().env({
        GITHUB_TOKEN: {
    ...
    const pipeline = Pipeline.builder().steps([
      CommandStep.builder()
        .label(":typescript: check:workspaces (back-end)")
        .command([
          "set -e",
      ...
      CommandStep.builder()
        ...
        .command(["npm install -g codecov", "./scripts/upload-coverage.sh"])
        .plugins([
          {
            ["artifacts#v1.5.0"]: {
              download: ["**/*"],
          ...
          DockerPlugin.builder()
            ...
            .environment(["FORCE_CLOUR=2", "CODECOV_TOKEN"]),
        ])
        .softFail(true),
      CommandStep.builder()
        .label(":jest: test:unit (back-end)")
